...
после — вызов выполняется. Человеку нужны минуты, а встроенный цикл повторов официального
Python SDK сдаётся примерно через две секунды опроса с `InputRequiredRoundsExceededError`.
Ловите это исключение и повторяйте вызов позже с тем же `requestState`.
...
elicitation-канал ревизии 2026-07-28), не работает за гейтом `L2`: оба вопроса делили бы один
`requestState`, и каждый повтор превращался бы в новый запрос человеку и новый настоящий вызов.
Прокси отказывает с `mrtr.upstream_input_required`, как только upstream спросил: на dry run,
...
Юнит-тесты гоняются против фейкового FastMCP-сервера, в процессе и через настоящие сокеты. В
`e2e/` лежат три стенда на docker compose в сетях без выхода наружу. Каждый ведёт образ прокси
...
`e2e/postgres` запускает небольшой SDK-сервер с честным dry run на Postgres, две реплики прокси и
