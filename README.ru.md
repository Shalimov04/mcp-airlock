...
`dry_run.unsupported`, `principal.missing`, `protocol.<code>`, `mrtr.pending`, `mrtr.declined`,
...
списывает blast radius и только потом сжигает ключ. Сжигание — атомарная вставка в
хранилище, так что две реплики не выполнят одно подтверждение дважды. Отказ человека тоже
сжигает ключ.
...
способа его вызвать нет. Ответ `tools/list` кэшируется на столько, сколько сказал
upstream в `ttlMs`, отдельно на каждого principal; при `ttlMs: 0` он запрашивается на
каждый гейтируемый вызов.
...
`X-Airlock-Principal` или `X-Forwarded-User`, запишется рядом с одобрением.
...
заменяются на `[REDACTED]`, как и значения, похожие на bearer-токены, ключи `sk-`, ключи
GitHub и AWS, JWT. Та же редакция применяется к тексту, который видит подтверждающий.
...
Проверьте это сами, прежде чем ставить тул на `L1` или `L2`.

Заголовки `Mcp-Param-*` пересылаются как пришли. Если тул зеркалит `dry_run` в такой
заголовок, переписанное прокси тело перестанет с ним совпадать, и upstream отклонит
вызов. Это безопасное направление, но зеркалить `dry_run` в заголовок нельзя.
